...
import { upvoteReviewById } from "@/review/review.network";
import { useMutation, useQueryClient } from "@tanstack/react-query";
import { useSelector } from "react-redux";
import { useParams, useSearchParams } from "react-router-dom";

export const useUpvoteReviewById = () => {
  ...
  const { mutate: upvoteReviewByIdMutation, isPending } = useMutation({
    mutationFn: upvoteReviewById,

    onMutate: async ({ rid, uid }) => {
      if (reviewId) {
        ...
        await queryClient.setQueryData(
          ...
          (review: any) => {
            ...
            if (idExistsInUpvoterIds) {
              newUpvoterIds = newUpvoterIds.filter((id: any) => id !== uid);
            } else if (idExistsInDownvoterIds) {
              newDownvoterIds = newDownvoterIds.filter((id: any) => id !== uid);
              newUpvoterIds.push(uid);
            } else {
              newUpvoterIds.push(uid);
            }
  ...
  return { upvoteReviewByIdMutation, isPending };
};
