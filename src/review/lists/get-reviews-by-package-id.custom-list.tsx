...
import { useGetReviewsByPackageId } from "../hooks/read";
...
export const GetReviewsByPackageIdCustomList = () => {
  const { isMobile } = useSelector((state: RootState) => state.view);
  const { page } = useSelector((state: RootState) => state.review);
  const { reviews, isPending, isError, error } = useGetReviewsByPackageId();
