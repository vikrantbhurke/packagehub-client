...
import { useSearchReviewsByPackageId } from "../hooks/read";
import { CustomError } from "@/global/components/errors";
import { SeoComponent } from "@/global/components/reusables";
import { PaginationPlaceholder } from "@/global/components/placeholders";
import { RootState } from "@/global/states/store";

export const SearchReviewsByPackageIdCustomList = () => {
  const { isMobile } = useSelector((state: RootState) => state.view);
  const { page } = useSelector((state: RootState) => state.review);
  const { reviews, isPending, isError, error } = useSearchReviewsByPackageId();
